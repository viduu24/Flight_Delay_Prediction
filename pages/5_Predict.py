...
<style>
    ...
    div[data-testid="stSelectbox"] > div > div { background: #111827 !important; color: #e2e8f0 !important; }
...
MODEL_PATHS = {
    "Bagged Decision Trees": [
        Path("Code/Models/bagging_model.pkl"),
        Path("../../Code/Models/bagging_model.pkl"),
        Path("../Models/bagging_model.pkl"),
        Path("bagging_model.pkl"),
    ],
    "KNN (k=20, Manhattan)": [
        Path("Code/Models/knn_model.pkl"),
        Path("../../Code/Models/knn_model.pkl"),
...
bagging_model, bagging_path = load_model(MODEL_PATHS["Bagged Decision Trees"])
knn_model, knn_path = load_model(MODEL_PATHS["KNN (k=20, Manhattan)"])
...
def get_le_encode(val, vocab):
    """Simulate label encoder (sorted vocab → index)."""
...
def build_bagging_features(month, day_of_month, day_of_week, carrier, origin_city,
    ...
    carrier_enc = get_le_encode(carrier, CARRIERS)
    city_enc    = get_le_encode(origin_city, MAJOR_CITIES)
    state_enc   = get_le_encode(origin_state, STATES)
    season_enc  = get_le_encode(season, ["Fall","Spring","Summer","Winter"])
    return pd.DataFrame([{
        "month": month, "day_of_month": day_of_month, "day_of_week": day_of_week,
        "op_unique_carrier": carrier_enc, "origin_city": city_enc,
        "origin_state": state_enc, "Precipitation_mm": precipitation,
        "dep_time": dep_time_int, "Season": season_enc,
...
if not model_available:
    st.warning("""
    ⚠️ **No .pkl models found.** The prediction page will run with a demo heuristic until models are placed in the expected path.
    
    Expected locations (relative to repo root):
    - `Code/Models/bagging_model.pkl`
    - `Code/Models/knn_model.pkl`
    """)
...
with col_result:
    ...
    if predict_btn:
        ...
        try:
            ...
            st.info("The model's expected feature schema may differ from the inputs built here. Check that the .pkl was saved with the same feature columns.")
            prob_delay = None
    ...
    Post-departure realized values (actual delay minutes, reason codes) are excluded to prevent leakage.
    The model's overall accuracy is ~72% with ROC-AUC ~0.747.
    </div>
    """, unsafe_allow_html=True)
